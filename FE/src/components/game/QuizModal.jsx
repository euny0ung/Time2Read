...
const QuizModal = React.memo(
  ({ quizIndex }) => {
    ...
    return (
      <div
        className="w-[85%] h-[80%] absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 gap-4 p-6 bg-white bg-opacity-50 border-2 border-white shadow-xl rounded-2xl flex flex-col items-start justify-center"
      > 
          {quiz.map((it, index) => {
            let questionText = ''
            ...
            const renderQuiz = (type, additionalProps = {}) => (
              <div className='w-[100%] h-[100%] flex items-center justify-center'>
                <div className='w-[65%] h-[100%] m-2 flex'>
                  <WhiteContainer>
                    <div className='w-[100%] h-[10%] flex items-center justify-center'>
                      <p className="text-2xl text-black"> Q{index+1}){questionText}</p>
                    </div>
                    {/* 질문 생성*/}
                    ...
                    {/* 정답 입력 생성 */}
                    <div className='flex justify-center items-center' style={{ width: '100%', height: `${answerHeight}%` }}>
                ...
                <div className='w-[35%] h-[100%] m-2 flex flex-col items-end justify-between'>
                  <div className='flex items-center justify-center overflow-y-scroll' style={{ width: '100%', height: `${hintOneHeight}%` }}>
                    <WhiteContainer>
                    <div className='w-full h-full'>
                      <EntireContentButton title={it.title} clues={it.quiz.clues[0]} quizIndex={quizIndex} />
                    </div>
                    </WhiteContainer>
                  </div>
                  ...
                  {
                    hintTwoHeight != 0 &&
                    <div className='flex items-center justify-center' style={{ width: '100%', height: `${hintTwoHeight}%` }}>
                      <WhiteContainer>
                      <div>
